...
public class AgentController {
        ...
        return agentService.chatStream(agentId, conversationId, message)
                .map(token -> {
                    try {
                        ...
                        return "data: {\"type\":\"error\",\"content\":\"Serialization error\"}\n\n";
                    }
                });
    }
    ...
    @PutMapping("/{agentId}")
    public ResponseEntity<Agent> updateAgent(@AuthenticationPrincipal UserPrincipal userPrincipal, @PathVariable String agentId, @RequestBody Agent agentDetails) {
        return ResponseEntity.ok(agentService.updateAgent(agentId, userPrincipal.getId(), agentDetails));
    }

    @DeleteMapping("/{agentId}")
    public ResponseEntity<Void> deleteAgent(@AuthenticationPrincipal UserPrincipal userPrincipal, @PathVariable String agentId) {
