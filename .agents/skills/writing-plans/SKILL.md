---
name: writing-plans
description: 在动手编写代码之前，当你有了多步任务的规范或要求时使用。 (Use when you have a spec or requirements for a multi-step task, before touching code)
---

# 撰写计划 (Writing Plans)

> [!NOTE]
> **中文摘要**：计划应假设执行者完全没有项目背景。将任务分解为每一项 2-5 分钟即可完成的“原子级”步骤。每一步必须包含具体的文件路径、代码、测试命令和预期输出。严禁使用任何占位符。
...
- Announce: "计划已完成并保存至 `docs/plans/<filename>.md`。自主模式：直接进入执行阶段。"
- Automatically invoke `executing-plans` to begin implementation

**If NOT in autonomous mode:**
- Announce: "计划已完成并保存至 `docs/plans/<filename>.md`。请使用 `executing-plans` 执行它。"
